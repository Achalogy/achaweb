...
export default function BlogLayout({children, isBlog}: { children: any, isBlog: boolean }) {
  ...
  return <>
    <div className="flex flex-col min-h-screen dark:bg-darkMode-900 items-center">
      ...
      <header className={`py-4 px-8 lg:px-4 flex gap-20 items-center ${isBlog ? "justify-between lg:justify-around w-full lg:w-7/12" : "justify-center"}`}>
        {isBlog && <FontAwesomeIcon icon={faChevronLeft} className="dark:text-white cursor-pointer" onClick={() => router.push("/blog")} />}
        
        {
          !isBlog && <div className="flex flex-row justify-center items-center gap-2" onClick={() => router.push("/")}>
            <FontAwesomeIcon icon={faChevronLeft} className="dark:text-white cursor-pointer"/>
            <h1 className="dark:text-white cursor-pointer font-semibold text-lg">Home</h1>
          </div>
        }
