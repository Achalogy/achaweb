...
export default function VideoComponent({ title, video, id }: Video) {
  ...
  return(
    <div
      ...
      className="w-10/12 lg:w-1/4 bg-white dark:bg-darkMode-700 rounded-lg drop-shadow-md cursor-pointer hover:brightness-110">
      <div className="aspect-video bg-gray-300 relative overflow-hidden">
        <Image src={miniature} fill alt={title}/>
      </div>
      <div className="p-4 text-center">
        <p className="font-semibold dark:text-zinc-300">{title}</p>
      </div>
    </div>
  )
}